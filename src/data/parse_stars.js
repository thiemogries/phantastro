...
function getStarColor(spectralType, bMag, vMag) {
  ...
  if (spectralType) {
    ...
    // Wolf-Rayet stars (blue)
      ...
      specType.includes('WR') ||
      specType.includes('WC') ||
      specType.includes('WN')
    ) {
...
function parseStarsFile(filename) {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      console.error(
        `Error parsing line: ${trimmedLine.substring(0, 50)}... - ${e.message}`
      );
    }
  }
...
function main() {
  ...
  console.log(
    `Magnitude range: ${output.magnitude_range.min} to ${output.magnitude_range.max}`
  );
  ...
  for (const mag of Object.keys(magnitudeCounts).sort(
