...
function calculateHourAngle(date: Date, longitude: number): number {
  // Get day of year for equation of time correction
  ...
  // Calculate decimal hours from UTC
  const utcHours =
    date.getUTCHours() +
    date.getUTCMinutes() / 60 +
    date.getUTCSeconds() / 3600;

  // Apply equation of time correction (in hours)
  const equationOfTime = calculateEquationOfTime(dayOfYear) / 60;

  // Convert to local solar time accounting for longitude
  // Longitude offset: 15 degrees per hour, positive east of Greenwich
  const longitudeOffset = longitude / 15;
  const localSolarTime = utcHours + longitudeOffset + equationOfTime;
 ...
 * @param declination - Solar declination angle in degrees
