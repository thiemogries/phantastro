...
class WeatherService {
  ...
  constructor() {
    ...
    this.baseUrl = process.env.REACT_APP_METEOBLUE_BASE_URL || 'https://my.meteoblue.com/packages';

    if (!this.apiKey) {
      console.warn('Meteoblue API key not found. Please set REACT_APP_METEOBLUE_API_KEY in your .env file');
    }
  }
   ...
   */
  async getWeatherForecast(lat: number, lon: number, locationName?: string): Promise<WeatherForecast> {
    try {
  ...
  async searchLocations(query: string): Promise<LocationSearchResult[]> {
    try {
      ...
      const commonLocations: LocationSearchResult[] = [
        { name: 'Mauna Kea, Hawaii', country: 'USA', lat: 19.8207, lon: -155.4680, elevation: 4207 },
        { name: 'Atacama Desert, Chile', country: 'Chile', lat: -24.6282, lon: -70.4044, elevation: 2400 },
        { name: 'La Palma, Spain', country: 'Spain', lat: 28.7636, lon: -17.8915, elevation: 2396 },
        { name: 'Mount Wilson, California', country: 'USA', lat: 34.2258, lon: -118.0569, elevation: 1742 },
        { name: 'Pic du Midi, France', country: 'France', lat: 42.9363, lon: 0.1415, elevation: 2877 },
        { name: 'Zurich, Switzerland', country: 'Switzerland', lat: 47.3769, lon: 8.5417, elevation: 408 },
        { name: 'New York, USA', country: 'USA', lat: 40.7128, lon: -74.0060, elevation: 10 },
        { name: 'London, UK', country: 'UK', lat: 51.5074, lon: -0.1278, elevation: 11 },
        { name: 'Tokyo, Japan', country: 'Japan', lat: 35.6762, lon: 139.6503, elevation: 40 },
        { name: 'Sydney, Australia', country: 'Australia', lat: -33.8688, lon: 151.2093, elevation: 58 }
      ];
  ...
  calculateObservingConditions(forecast: HourlyForecast): ObservingConditions {
    const cloudScore = Math.max(0, 10 - (forecast.cloudCover.totalCloudCover / 10));
    ...
    const transparencyScore = Math.max(0, 10 - (forecast.humidity / 10) - (forecast.cloudCover.totalCloudCover / 20));

    const overallScore = (cloudScore * 0.4 + seeingScore * 0.3 + transparencyScore * 0.2 + windScore * 0.1);
  ...
  private async fetchBasicWeatherData(lat: number, lon: number): Promise<any> {
    ...
    const response = await axios.get(`${this.baseUrl}/basic-1h_basic-day`, { params });
  ...
  private async fetchAstronomyData(lat: number, lon: number): Promise<any> {
    ...
    try {
      const response = await axios.get(`${this.baseUrl}/astro-1h_astro-day`, { params });
      return response.data;
    } catch (error) {
      console.warn('Astronomy data not available, using basic data only');
      return null;
    }
  }
   ...
   */
  private transformMeteoblueData(data: any, location: Location): WeatherForecast {
    const now = new Date();
    ...
    if (data.data_1h) {
      for (let i = 0; i < Math.min(72, data.data_1h.time.length); i++) { // 3 days of hourly data
    ...
    if (data.data_day) {
      for (let i = 0; i < Math.min(7, data.data_day.time.length); i++) { // 7 days
        ...
        dailyForecast.push({
          ...
          precipitationProbability: data.data_day.precipitation_probability_max[i],
   ...
   */
  private enhanceWithAstronomyData(dailyForecast: DailyForecast[], astronomyData: any): DailyForecast[] {
  ...
  private getMockWeatherData(lat: number, lon: number): any {
    ...
    return {
      ...
      data_1h: {
        ...
        precipitation_probability: precipitation.map(p => p > 0 ? 60 + Math.random() * 40 : Math.random() * 30)
      },
