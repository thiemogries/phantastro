...
class WeatherService {
  ...
  async searchLocations(
    ...
    try {
      ...
      const response = await axios.get(nominatimUrl, {
        ...
        signal, // Use the signal passed from TanStack Query
        headers: {
          'User-Agent':
            'PhantAstro Weather App (https://github.com/user/phantastro)',
        },
      });
  ...
  async fetchCloudData(lat: number, lon: number): Promise<any> {
    ...
    try {
      ...
      console.warn(
        `⚠️ Failed to fetch cloud data, will use basic weather fallback:`,
  ...
  async fetchMoonlightData(lat: number, lon: number): Promise<any> {
    ...
    try {
      ...
      console.warn(
        `⚠️ Failed to fetch moonlight data, continuing without moonlight information:`,
        error instanceof Error ? error.message : 'Unknown error'
      );
  ...
  async fetchSunMoonData(lat: number, lon: number): Promise<any> {
    ...
    try {
      ...
      console.warn(
        `⚠️ Failed to fetch sun/moon data, continuing without sun/moon information:`,
        error instanceof Error ? error.message : 'Unknown error'
      );
  ...
  transformMeteoblueData(
    ...
    if (
      !data.data_1h.time ||
      !Array.isArray(data.data_1h.time) ||
      data.data_1h.time.length === 0
    ) {
    ...
    if (cloudData?.data_1h) {
      ...
      for (const [fieldName, array] of Object.entries(cloudArrays)) {
        if (array && array.length !== timeLength) {
          console.warn(
            `⚠️ Cloud data length mismatch for ${fieldName}: expected ${timeLength}, got ${array.length}`
          );
        }
      }
    }
    ...
    for (const [fieldName, array] of Object.entries(dataArrays)) {
      if (array && array.length !== timeLength) {
        console.warn(
          `⚠️ Length mismatch for ${fieldName}: expected ${timeLength}, got ${array.length}`
        );
      }
    }
    ...
    for (let i = 0; i < availableHours; i++) {
      const precipitationData: PrecipitationData = {
        ...
        precipitationProbability:
          data.data_1h.precipitation_probability?.[i] ?? null,
      };

      // Validate precipitation data
      if (
        precipitationData.precipitation !== null &&
        precipitationData.precipitation < 0
      ) {
        console.warn(
          `⚠️ Invalid precipitation value at hour ${i}: ${precipitationData.precipitation}`
        );
        precipitationData.precipitation = 0;
      }
      if (
        precipitationData.precipitationProbability !== null &&
        (precipitationData.precipitationProbability < 0 ||
          precipitationData.precipitationProbability > 100)
      ) {
        console.warn(
          `⚠️ Invalid precipitation probability at hour ${i}: ${precipitationData.precipitationProbability}`
        );
        precipitationData.precipitationProbability = Math.max(
          0,
          Math.min(100, precipitationData.precipitationProbability)
        );
      }
      ...
      const hourData = {
        ...
        cloudCover: {
          totalCloudCover: (() => {
            ...
            const cloudValue = safeExtractValue(
              cloudData?.data_1h?.totalcloudcover,
              i
            );
        ...
        precipitation: {
          ...
          precipitationProbability: safeExtractValue(
            data.data_1h.precipitation_probability,
            i
          ),
        },
      ...
      if (
        ...
        (hourData.temperature < -100 || hourData.temperature > 60)
      ) {
        console.warn(
          `⚠️ Extreme temperature value at hour ${i}: ${hourData.temperature}°C`
        );
      }
      if (
        hourData.windSpeed !== null &&
        (hourData.windSpeed < 0 || hourData.windSpeed > 200)
      ) {
        console.warn(
          `⚠️ Invalid wind speed at hour ${i}: ${hourData.windSpeed} m/s`
        );
    ...
    if (availableHours < targetHours && availableHours > 0) {
      ...
      for (let i = availableHours; i < targetHours; i++) {
        const forecastTime = new Date(
          baseTime.getTime() + (i - availableHours + 1) * 60 * 60 * 1000
        );
        ...
        hourlyForecast.push({
          ...
          temperature:
            lastRealHour.temperature !== null
              ? lastRealHour.temperature + tempVariation
              : null,
          humidity:
            lastRealHour.humidity !== null
              ? Math.max(
                  20,
                  Math.min(
                    100,
                    lastRealHour.humidity + (Math.random() - 0.5) * 10
                  )
                )
              : null,
          windSpeed:
            lastRealHour.windSpeed !== null
              ? Math.max(0, lastRealHour.windSpeed + windVariation)
              : null,
          windDirection:
            lastRealHour.windDirection !== null
              ? (lastRealHour.windDirection + (Math.random() - 0.5) * 45) % 360
              : null,
          cloudCover: {
            totalCloudCover:
              lastRealHour.cloudCover.totalCloudCover !== null
                ? Math.max(
                    0,
                    Math.min(
                      100,
                      lastRealHour.cloudCover.totalCloudCover + cloudVariation
                    )
                  )
                : null,
            lowCloudCover:
              lastRealHour.cloudCover.lowCloudCover !== null
                ? Math.max(
                    0,
                    Math.min(
                      100,
                      lastRealHour.cloudCover.lowCloudCover +
                        cloudVariation * 0.3
                    )
                  )
                : null,
            midCloudCover:
              lastRealHour.cloudCover.midCloudCover !== null
                ? Math.max(
                    0,
                    Math.min(
                      100,
                      lastRealHour.cloudCover.midCloudCover +
                        cloudVariation * 0.4
                    )
                  )
                : null,
            highCloudCover:
              lastRealHour.cloudCover.highCloudCover !== null
                ? Math.max(
                    0,
                    Math.min(
                      100,
                      lastRealHour.cloudCover.highCloudCover +
                        cloudVariation * 0.3
                    )
                  )
                : null,
          ...
          moonlight: {
            moonlightActual:
              lastRealHour.moonlight?.moonlightActual !== null &&
              lastRealHour.moonlight?.moonlightActual !== undefined
                ? Math.max(
                    0,
                    Math.min(
                      100,
                      lastRealHour.moonlight.moonlightActual +
                        (Math.random() - 0.5) * 5
                    )
                  )
                : null,
            moonlightClearSky:
              lastRealHour.moonlight?.moonlightClearSky !== null &&
              lastRealHour.moonlight?.moonlightClearSky !== undefined
                ? Math.max(
                    0,
                    Math.min(
                      100,
                      lastRealHour.moonlight.moonlightClearSky +
                        (Math.random() - 0.5) * 3
                    )
                  )
                : null,
            nightSkyBrightnessActual:
              lastRealHour.moonlight?.nightSkyBrightnessActual !== null &&
              lastRealHour.moonlight?.nightSkyBrightnessActual !== undefined
                ? Math.max(
                    0,
                    lastRealHour.moonlight.nightSkyBrightnessActual +
                      (Math.random() - 0.5) * 50
                  )
                : null,
            nightSkyBrightnessClearSky:
              lastRealHour.moonlight?.nightSkyBrightnessClearSky !== null &&
              lastRealHour.moonlight?.nightSkyBrightnessClearSky !== undefined
                ? Math.max(
                    0,
                    lastRealHour.moonlight.nightSkyBrightnessClearSky +
                      (Math.random() - 0.5) * 20
                  )
                : null,
            zenithAngle:
              lastRealHour.moonlight?.zenithAngle !== null &&
              lastRealHour.moonlight?.zenithAngle !== undefined
                ? Math.max(
                    0,
                    Math.min(
                      360,
                      lastRealHour.moonlight.zenithAngle +
                        (Math.random() - 0.5) * 3
                    )
                  )
                : null,
          ...
          visibility:
            lastRealHour.visibility !== null &&
            lastRealHour.visibility !== undefined
              ? Math.max(5, lastRealHour.visibility + (Math.random() - 0.5) * 5)
              : null,
        });
      }
    }
      ...
      for (let day = 0; day < 7; day++) {
        ...
        if (dayHours.length === 0) {
          ...
          const clouds = dayHours
            .map(h => h.cloudCover.totalCloudCover)
            .filter(c => c !== null) as number[];
          const windSpeeds = dayHours
            .map(h => h.windSpeed)
            .filter(w => w !== null) as number[];

          const precips = dayHours
            .map(h => h.precipitation.precipitation)
            .filter(p => p !== null) as number[];
          const precipProbs = dayHours
            .map(h => h.precipitation.precipitationProbability)
            .filter(p => p !== null) as number[];

          const cloudAvg =
            clouds.length > 0
              ? clouds.reduce((sum, c) => sum + c, 0) / clouds.length
              : 50;
          const maxPrecipProb =
            precipProbs.length > 0 ? Math.max(...precipProbs) : 0;
          const totalPrecip =
          ...
          if (sunMoonData?.data_day) {
            ...
            if (dayIndex !== -1 && dayIndex !== undefined) {
              sunMoonDataForDay = {
                ...
                moonrise:
                  sunMoonData.data_day.moonrise?.[dayIndex] === '---'
                    ? null
                    : sunMoonData.data_day.moonrise?.[dayIndex] || null,
                moonset:
                  sunMoonData.data_day.moonset?.[dayIndex] === '---'
                    ? null
  ...
  private checkDataIntegrity(hourlyForecast: HourlyForecast[]): void {
    ...
    for (let i = 1; i < hourlyForecast.length; i++) {
      ...
      if (Math.abs(actualDiff - expectedDiff) > 60000) {
        // Allow 1 minute tolerance
        timeGaps.push(
          `Gap at index ${i}: ${actualDiff / 1000 / 60} minutes instead of 60`
        );
      }
    }
